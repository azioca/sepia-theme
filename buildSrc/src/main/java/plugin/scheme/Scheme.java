...
public class Scheme {
	...
	class Colors {
		...
		List<Option.Color> option = List.of(
			...
			new Option.Color("INDENT_GUIDE", palette.gray().darker(2)),
			...
			new Option.Color("TEARLINE_COLOR", palette.gray().darker(2)),
	...
	class Attributes {
		...
		List<Attribute> option = List.of(
			...
			new Attribute("CTRL_CLICKABLE").foreground(palette.blue().darker()).effectColor(palette.blue().darker()).boldUnderscored(),
			...
			new Attribute("DEFAULT_INVALID_STRING_ESCAPE").foreground(foreground).effectColor(palette.red()).underwaved(),
			...
			new Attribute("DEFAULT_REASSIGNED_LOCAL_VARIABLE").foreground(palette.purple()).effectColor(palette.purple()).boldUnderscored(),
			new Attribute("DEFAULT_REASSIGNED_PARAMETER").foreground(palette.purple()).bold().effectColor(palette.purple()).boldUnderscored(),
			...
			new Attribute("DEPRECATED_ATTRIBUTES").foreground(palette.gray()).italic().strikeout(),
			new Attribute("DOC_COMMENT_TAG_VALUE").baseAttributes("DEFAULT_DOC_COMMENT_TAG_VALUE"),
			new Attribute("ERRORS_ATTRIBUTES").foreground(palette.red()).errorStripeColor(palette.red()).effectColor(palette.red()).underwaved(),
			new Attribute("FOLDED_TEXT_ATTRIBUTES").foreground(foreground).background(palette.aqua().brighter()),
			new Attribute("FOLLOWED_HYPERLINK_ATTRIBUTES").foreground(palette.blue()).effectColor(palette.blue()).boldUnderscored(),
			new Attribute("DELETED_TEXT_ATTRIBUTES").effectColor(palette.red()).errorStripeColor(palette.red()).dottedLine(),
			...
			new Attribute("INFO_ATTRIBUTES").effectColor(palette.yellow().darker()).errorStripeColor(palette.yellow().darker()).dottedLine(),
			...
			new Attribute("MARKED_FOR_REMOVAL_ATTRIBUTES").effectColor(palette.red().darker()).strikeout(),
			new Attribute("MATCHED_BRACE_ATTRIBUTES").foreground(foreground).bold(),
			new Attribute("NOT_USED_ELEMENT_ATTRIBUTES").foreground(palette.gray()).underwaved(),
			new Attribute("RUNTIME_ERROR").effectColor(palette.red().brighter()).errorStripeColor(palette.red().brighter()).underwaved(),
			...
			new Attribute("TYPO").effectColor(palette.gray().brighter()).underwaved(),
			...
			new Attribute("WARNING_ATTRIBUTES").effectColor(palette.yellow().darker()).errorStripeColor(palette.yellow().darker()).underwaved(),
			...
			new Attribute("WRONG_REFERENCES_ATTRIBUTES").effectColor(palette.red()).errorStripeColor(palette.red()).underwaved()
		);
	}
}
